#ifndef TESTBOXMODELWITHNOPROLIFERATIONANDVARYINGESSTIFFNESS_HPP_
#define TESTBOXMODELWITHNOPROLIFERATIONANDVARYINGESSTIFFNESS_HPP_
...
class TestBoxModelNoDivisionAndVaryingESStiffness : public AbstractCellBasedTestSuite
{
public:
	void TestVaryingESStiffness() throw(Exception)
	{
		...
		for (double SS = 4.0; SS < 7.0; SS++)
		{
			...
			for (double BM = 0.0; BM < 16.0; BM ++)
			{
				...
				for (double EE = 0.0; EE < 41.0; EE ++)
				{
					...
					std::string output_directory = "BoxModelVaryingESStiffness/" + out.str();
