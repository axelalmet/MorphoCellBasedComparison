...
{
public:
	void TestGrowingEpithelium() throw(Exception)
	{
		...
		for (double BM = 11.0; BM < 12.0; BM ++)
		{
			...
			for (double EE = 5.0; EE < 6.0; EE ++)
			{
				...
				out << "B_" << bm_stiffness << "/L0_" << epithelial_epithelial_resting_spring_length;
				std::string output_directory = "BoxModel/" + out.str();
