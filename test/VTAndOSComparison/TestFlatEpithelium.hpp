#ifndef TESTFLATEPITHELIUM_HPP_
#define TESTFLATEPITHELIUM_HPP_
...
static const std::string M_OUTPUT_DIRECTORY = "FlatEpithelium";
static const double M_DT = 0.005;
static const double M_END_TIME = 1.0;
static const double M_SAMPLING_TIMESTEP = M_END_TIME/M_DT;

class TestFlatEpithelium : public AbstractCellBasedTestSuite
{
public:
	void TestVTEpithelium()
	{
		...
		for (unsigned i = 0; i<location_indices.size(); i++)
		{
			...
			if (y != max_height)
			{
				...
				p_cycle_model->SetMinCellCycleDuration(11.0);
				p_cycle_model->SetMaxCellCycleDuration(13.0);
		...
		out << "/VT/B_" << bm_stiffness << "_TC_" << target_curvature;
		...
		p_bm_force->ApplyForceToCrypt(false);
