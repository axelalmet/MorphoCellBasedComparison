#ifndef TESTBOXMODELWITHNOPROLIFERATIONANDVARYINGEESTIFFNESS_HPP_
#define TESTBOXMODELWITHNOPROLIFERATIONANDVARYINGEESTIFFNESS_HPP_
...
class TestBoxModelNoDivisionAndVaryingEEStiffness : public AbstractCellBasedTestSuite
{
public:
	void TestVaryingEEStiffness() throw(Exception)
	{
		...
		for (double SS = 1.0; SS < 6.0; SS++)
		{
			...
			for (double BM = 0.0; BM < 16.0; BM ++)
			{
				...
				for (double EE = 0.0; EE < 41.0; EE ++)
				{
					...
					out << "EE_" << epithelial_epithelial_stiffness << "/B_" << bm_stiffness 
									<< "/L0_" << epithelial_epithelial_resting_spring_length;
					std::string output_directory = "BoxModelVaryingEEStiffness/" + out.str();
...
#endif /* TESTBOXMODELWITHGROWINGEPITHELIUM_HPP_ */
