...
public:
	void TestFlatEpithelium()
	{
		...
		double end_time = 0.0; //Set end time
		double sampling_timestep = end_time/dt;

		double second_end_time = 2.0*end_time;
		...
		double vertical_force_magnitude = 5.0;
...
//		MutableMesh<2,2>* p_mesh = generator.GetMesh();
		...
		// Now unload the force and run the simulation again
        boost::shared_ptr<VerticalCompressionForce > p_vertical_force_from_simulation =
                boost::static_pointer_cast<VerticalCompressionForce >(simulator.rGetForceCollection()[2]);
        p_vertical_force_from_simulation->SetForceMagnitude(0.0);

		simulator.SetEndTime(second_end_time);
...
//		//Set output directory
//		std::stringstream out;
//		out << "/CRYPT/B_" << bm_stiffness << "_TC_" << target_curvature << "/L0_" << epithelial_epithelial_resting_spring_length;
//		std::string output_directory = M_OUTPUT_DIRECTORY + out.str();
//		simulator.SetOutputDirectory(output_directory);
//
//		simulator.SetDt(dt);
