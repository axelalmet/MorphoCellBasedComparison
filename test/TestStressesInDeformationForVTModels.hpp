...
public:
	...
	void TestBuckledCryptEpithelium()
	{
		...
		simulator.SetDt(M_DT);
		simulator.SetSamplingTimestepMultiple(steady_state_time/M_DT); //Sample the simulation at every hour
		...
		for (double vert = -1.0; vert < 0.0; vert += 1.0)
		{
			...
			p_simulator->SetEndTime(steady_state_time + M_END_TIME); //Hopefully this is long enough for a steady state
			...
			p_simulator->SetEndTime(steady_state_time + M_END_TIME + M_SECOND_END_TIME); // Set new end time
			...
			//Tidying up
			SimulationTime::Instance()->Destroy();
			SimulationTime::Instance()->SetStartTime(10.0);
