#ifndef TESTBOXMODELWITHNOPROLIFERATIONANDVARYINGSTROMASTIFFNESS_HPP_
#define TESTBOXMODELWITHNOPROLIFERATIONANDVARYINGSTROMASTIFFNESS_HPP_
...
class TestBoxModelNoDivisionAndVaryingStromaStiffness : public AbstractCellBasedTestSuite
{
public:
	void TestVaryingStroma() throw(Exception)
	{
		...
		for (double ES = 1.0; ES < 19.0; ES++)
		{
            double epithelial_stromal_stiffness = 5.0*ES;
            double stromal_stromal_stiffness = epithelial_epithelial_stiffness;
			...
			for (double BM = 0.0; BM < 16.0; BM ++)
			{
				...
				for (double EE = 0.0; EE < 41.0; EE ++)
				{
					...
					out << "ES_" << epithelial_stromal_stiffness << "/B_" << bm_stiffness
									<< "/L0_" << epithelial_epithelial_resting_spring_length;
					std::string output_directory = "BoxModelVaryingFoundationStiffness/" + out.str();
...
#endif /* TESTBOXMODELWITHNODIVISIONANDVARYINGSTROMASTIFFNESS_HPP_ */
